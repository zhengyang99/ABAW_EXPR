...
class ContinuousMetricsCalculatorTrial(object):
    ...
    def calculator(output, label, metric):
        if metric == "rmse":
            ...
            result = f1(output, label)

        elif metric == "pcc":
            #result = pearsonr(output, label)
            result = [f1(output, label),f1(output, label)]
    ...
    def calculate_metrics(self):
        ...
        # partitionwise_output_copy=copy.deepcopy(trialwise_output)


        # for key,value in trialwise_output.items():
        #         smooth_array=sliding_window_mode(np.array(value['Arousal']),30)
        #         trialwise_output[key]['Arousal']=smooth_array
        ...
        ##计算指标平均
        # acc_all=[]
        # for key,value in self.metric_record_dict.items():
        #     f1_all.append(value['Arousal']['ccc'][0])
        
        # print('acc_score:',sum(f1_all)/len(f1_all))
...
def f1(pred,gt):
    ...
    for i in range(class_num):
        gt_ = gt[:, i]
        pred_ = pred[:, i]
        F1.append(f1_score(gt_, pred_,zero_division=0))
    F1_mean = np.mean(F1)
    return F1_mean

def sliding_window_mode(labels, window_size=5):
    """
    使用滑窗众数方法进行平滑处理。
    
    :param labels: 一个形状为 (num_frames,) 的数组，表示每帧的预测分类标签。
    ...
    for i in range(num_frames):
        ...
        # 计算窗口内标签的众数
        window_mode = mode(labels[start:end])[0][0]
        
        smoothed_labels[i] = window_mode
