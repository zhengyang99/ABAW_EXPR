...
class ABAW8Trainer(object):
    ...
    def loop(self, data_loader, epoch, train_mode=True):
        ...
        for batch_index, (X, Y, trials, lengths, indices) in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            if len(X.keys())>1:
                outputs = self.model(inputs, inputs4, inputs3) # [8.300.2]
            else:
            ...
            #     tmp[index]=sliding_window_mode(tmp[index],window_size=50)
            # output_handler.update_output_for_seen_trials(tmp, trials, indices, lengths)
    ...
    def loop_test(self, data_loader, output_save_path):
        ...
        for batch_index, (X, Y, trials, lengths, indices) in tqdm(enumerate(data_loader), total=len(data_loader)):
            total_batch_counter += len(trials)
            # Y:tensor([0, 0, 0, 0, 0, 0, 0, 0]) shape:[8]
            if 'frame' in X:
                inputs = X['frame'].to(self.device)
...
# def sliding_window_mode(labels, window_size=10):
#     """
#     使用滑窗众数方法进行平滑处理。
