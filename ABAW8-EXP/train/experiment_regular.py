...
from model.model import my_2d1d, my_2d1ddy, my_model2
...
class CCCLoss(nn.Module):
    ...
    def forward(self, pred, gold, weights=None):
        ...
        pred=pred.view(-1,8) #torch.Size([4800, 8])
        gold=gold.view(-1).type(torch.int64)
        # gold = torch.randint(0, 8, (4800,)).to(pred.device)
        # print(str(pred.shape)+"---"+str(gold.shape))
        ll = self.ce(pred,gold)
        return ll
...
class Experiment(object):
    ...
    def init_dataloader(self, fold):
        ...
        train_dataset = ABAW8_VA_Dataset(data_dict['Train_Set'], time_delay=self.time_delay, emotion=self.train_emotion,
        ...
        validate_dataset = ABAW8_VA_Dataset(data_dict['Validation_Set'], time_delay=self.time_delay,
    ...
    def experiment(self):
        ...
        for fold in iter(self.folds_to_run):
            ...
            trainer = ABAWTrainer(model, model_name=self.model_name, learning_rate=self.learning_rate,
